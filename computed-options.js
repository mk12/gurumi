module.exports = function (themeConfig) {
  ...
  let fontBody = themeConfig.customConfig.find(
    (option) => option.name === "fontBody",
  ).value;
  let fontHeadings = themeConfig.customConfig.find(
    (option) => option.name === "fontHeadings",
  ).value;

  let disableFontBodyItalic = themeConfig.customConfig.find(
    (option) => option.name === "disableFontBodyItalic",
  ).value;
  let disableFontHeadingsItalic = themeConfig.customConfig.find(
    (option) => option.name === "disableFontHeadingsItalic",
  ).value;
  ...
  return [
    {
      ...
      type: "checkbox",
      value:
        !disableFontBodyItalic && (fontParams[fontBody]?.hasItalic || false),
    },
    {
      name: "fontHeadingsItalic",
      type: "checkbox",
      value:
        !disableFontHeadingsItalic &&
        (fontParams[fontHeadings]?.hasItalic || false),
    },
  ];
};
